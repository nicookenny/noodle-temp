...
export const ModuleCard: FC<ModuleCardProps> = ({
  ...
  const moduleColor =
    ...
    const donePercentage =  tasks.length ? ( done.length /  tasks.length) * 100 : 100
    ...
    <li className="shrink-0 basis-full lg:basis-[275px]">
      ...
      >
        ...
        <div className="mt-3 flex flex-col gap-2">
          <p className="text-tiny text-default-500">{undone.length} Tasks remaining</p>
          <div className="relative h-2">
            <div className="absolute left-0 top-0 h-2 w-full overflow-hidden rounded-full bg-default-100" />
            <div
              className="absolute left-0 top-0 h-2 rounded-full"
              style={{ width: `${donePercentage}%`, background: moduleColor["500"] }}
            />
          </div>
        </div>
      </Link>
